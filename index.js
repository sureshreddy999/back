...
// //     'https://frontend-p8yq.vercel.app'// For local development of your frontend
// //   ],
// //   const vercelPreviewRegex = /^https:\/\/frontend-[a-z0-9]+\.vercel\.app$/;
...
// // ✅ Preflight OPTIONS handler (must be after)
...
// // /backend/index.js - COMPLETE REVISED CODE (CORS Issues Fixed)
...
// const app = express();
// const PORT = process.env.PORT || 8080; // Changed from 5001 to 8080 for Render compatibility
...
// // ✅ Preflight OPTIONS handler (must be after CORS)
// app.options('*', cors());
...
// // Debug middleware to log all incoming requests (MOVED BEFORE ROUTES - CRITICAL FIX)
// app.use((req, res, next) => {
//   console.log(`📝 ${new Date().toISOString()} - ${req.method} ${req.url}`);
//   console.log('📝 Origin:', req.get('Origin'));
//   console.log('📝 Headers:', req.headers);
...
// // Register API routes (MOVED AFTER DEBUG MIDDLEWARE)
// app.use('/api', uploadPhotoRoute);
// app.use('/api', userProfileRoute);
// app.use('/api', weeklyDietPlanRoute);
...
//   console.log(`🚀 Advanced Diet Plan Server running on http://0.0.0.0:${PORT}`);
//   // Check if Gemini API key is configured
//   console.log(`🔑 Gemini API Key configured: ${process.env.GEMINI_API_KEY ? 'Yes' : 'No'}`);
//   // Check if AWS credentials are set (not comprehensive, but a quick check)
//   console.log(`☁️ AWS Credentials configured: ${process.env.AWS_ACCESS_KEY_ID ? 'Yes' : 'No'}`);
...
// COMMENTED OUT ROUTE IMPORTS - Uncomment only if these files exist in your repo
// import uploadPhotoRoute from './routes/upload-profile-photo.js';
// import userProfileRoute from './routes/user-profile-photo.js';
// import weeklyDietPlanRoute from './routes/generate-diet-plan.js'; 
...
app.use(cors({
  origin: function (origin, callback) {
    const allowedOrigins = [
      'http://localhost:3000',
      'https://frontend-zula.vercel.app',
      'https://frontend-i7t4.vercel.app'  // ADDED YOUR NEW FRONTEND URL
    ];
...
// COMMENTED OUT ROUTE REGISTRATIONS - Uncomment only if route files exist
// app.use('/api', uploadPhotoRoute);
// app.use('/api', userProfileRoute);
// app.use('/api', weeklyDietPlanRoute);

// TEMPORARY ROUTE FOR TESTING USER PROFILE PHOTO
app.get('/api/user-profile-photo/:email', async (req, res) => {
  const { email } = req.params;
  
  console.log(`📸 Fetching profile photo for email: ${email}`);
  ...
  try {
    // For now, return a success response since AWS might not be configured
    res.json({
      success: true,
      image: null, // No image found
      message: 'Profile photo endpoint working - AWS S3 integration needed for actual photos'
    });
  } catch (error) {
...
// Setup Google Gemini model (only if API key exists)
let model = null;
...
// Setup AWS SDK for DynamoDB (only if credentials exist)
let dynamoDB = null;
...
app.post('/api/generate-diet-plan', async (req, res) => {
  ...
  if (!email || !age || !weight || !height || !activityLevel || !goal) {
    return res.status(400).json({ 
      success: false, 
      message: 'Missing required fields for diet plan generation.' 
    });
  }
  ...
  try {
    ...
    try {
      ...
      jsonPart = jsonPart.replace(/([{,]\s*)([a-zA-Z_][a-zA-Z0-9_]*)\s*:/g, '$1"$2":');
      jsonPart = jsonPart.replace(/,\s*([}\]])/g, '$1');
      jsonPart = jsonPart.replace(/: True/g, ': true').replace(/: False/g, ': false');
    ...
    for (const day of days) {
      const dayPrompt = `
        ...
        - Cuisine Preference: ${cuisinePreference}
        - Meals per day: ${mealCount}
        - Target Calories for this day: ${Math.round(nutrition.calories / 7)}
        - Target Protein: ${Math.round(nutrition.protein / 7)}g
        - Target Carbs: ${Math.round(nutrition.carbs / 7)}g
        - Target Fats: ${Math.round(nutrition.fats / 7)}g
      ...
      try {
        const dayResult = await chatDay.sendMessage(dayPrompt);
        dayRawText = dayResult.response.text();
        console.log(`💬 Gemini raw response for ${day} length:`, dayRawText.length);

        const dayJsonMatches = dayRawText.match(/```json\s*([\s\S]*?)```/) ||
                               dayRawText.match(/```\s*([\s\S]*?)```/) ||
                               dayRawText.match(/\[[\s\S]*\]/);
        ...
        dayJsonPart = dayJsonPart.replace(/: ([^",\s\{\}\[\]]+)(?=[,}\]])/g, ': "$1"');
        dayJsonPart = dayJsonPart.replace(/: '([^']+)'/g, ': "$1"');
        dayJsonPart = dayJsonPart.replace(/\\'/g, "'");
        dayJsonPart = dayJsonPart.replace(/\\"/g, '"');
        dayJsonPart = dayJsonPart.replace(/[-\u001F\u007F-\u009F]/g, '');
        dayJsonPart = dayJsonPart.replace(/[\u200B-\u200D\uFEFF]/g, '');
        dayJsonPart = dayJsonPart.replace(/[\u00A0]/g, ' ');
        ...
        console.error(`❌ Error generating or parsing plan for ${day}:`, dayErr);
        console.error(`Raw response for ${day} (full):`, dayRawText);
        weeklyPlanGenerated[day] = [];
      }
    }
    ...
    // Save to DynamoDB only if configured
    if (dynamoDB) {
      const timestamp = new Date().toISOString();
      await dynamoDB.put({
        TableName: 'DietPlans',
...
// POST: Endpoint for general nutrition analysis
app.post('/api/analyze-nutrition', async (req, res) => {
  const { age, weight, height, activityLevel, goal } = req.body;
  ...
  if (!age || !weight || !height || !activityLevel || !goal) {
    return res.status(400).json({ 
      success: false, 
      message: 'Missing required fields for nutrition analysis.' 
    });
  }
